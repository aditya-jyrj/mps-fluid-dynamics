...
function standard_to_qtt_permutation(n::Int)
    ...
    for j in 1:M
        ...
        e_qtt = grid_to_qtt_vector(u_grid, n)
...
function standard_to_qtt_matrix(A::AbstractMatrix, n::Int)
    P = standard_to_qtt_permutation(n)
...
function grid_to_qtt_tensor(u::AbstractMatrix, n::Int)
    Nx, Ny = size(u)
...
function qtt_tensor_to_grid(T::AbstractArray, n::Int)
...
function grid_to_qtt_mps(u::AbstractMatrix, sites::Vector{<:Index}; cutoff=1e-10)
    ...
    T = grid_to_qtt_tensor(u, n)
    IT = ITensor(T, sites...)
    return MPS(IT, sites; cutoff=cutoff)
end

function qtt_mps_to_grid(mps::MPS, sites::Vector{<:Index})
    nsites = length(sites)
    iseven(nsites) || throw(ArgumentError("Need an even number of sites for 2D QTT"))
    n = nsites ÷ 2
    ...
    return qtt_tensor_to_grid(T, n)
end

# GRID <-> QTT VECTOR

function grid_to_qtt_vector(u::AbstractMatrix, n::Int)
    T = grid_to_qtt_tensor(u, n)
    return reshape(T, :)
end


function qtt_vector_to_grid(v::AbstractVector, n::Int)
    N = 2^n
    length(v) == N^2 || throw(ArgumentError("Expected vector of length $(N^2)"))
